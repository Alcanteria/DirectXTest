...
#include "..\Library\SkinnedModelMaterial.h"
#include "..\Library\VectorHelper.h"
#include "..\Library\ColorHelper.h"
#include "..\Library\AnimationPlayer.h"
#include "..\Library\AnimationClip.h"
...
namespace Rendering
{
	...
	void AnimationDemo::Initialize()
	{
		...
		// Load the model
		mSkinnedModel = new Model(*mGame, "..\\source\\Library\\Content\\Models\\RunningSoldier.dae", true);
		...
		for (UINT i = 0; i < mSkinnedModel->Meshes().size(); i++)
		{
			...
			if (material != nullptr && material->Textures().find(TextureTypeDifffuse) != material->Textures().cend())
			{
				...
				textureName << L"..\\source\\Library\\Content\\Models\\" << filename.substr(0, filename.length() - 4) << L".png";
	...
	void AnimationDemo::UpdateOptions()
	{
		if (mKeyboard != nullptr)
		{
			...
			if (mManualAdvanceMode && mKeyboard->WasKeyPressedThisFrame(DIK_H))
			{
