...
namespace Rendering
{
	...
	void ShadowMappingDemo::Initialize()
	{
		...
		std::wstring textureName = L"..\\source\\Library\\Content\\Textures\\Checkerboard.png";
		...
		std::unique_ptr<Model> model(new Model(*mGame, "..\\source\\Library\\Content\\Models\\teapot.obj", true));
	...
	void ShadowMappingDemo::UpdatePointLightAndProjector(const GameTime& gameTime)
	{
		...
		if (mKeyboard != nullptr)
		{
			...
			{
				movementAmount.x = -moveIncrement;
			}
			...
			{
				movementAmount.y = moveIncrement;
			}
			...
			{
				movementAmount.z = -moveIncrement;
			}
